...
train_pairs, val_pairs = train_test_split(dataset.pairs, test_size=0.2, random_state=42)
# invert for better training
...
def get_dataloader(batch_size):
    ...
    for idx, (inp, tgt) in enumerate(train_pairs):
        inp_ids = indexesFromSentence(char2int, inp)
        tgt_ids = indexesFromSentence(phon2int, tgt)
    ...
    with torch.no_grad():
        ...
        for idx in decoded_ids:
            if idx.item() == EOS_token:
                ...
                break
            decoded_words.append(int2phon[idx.item()])
...
def evaluateRandomly(encoder, decoder, n=10):
    for _ in range(n):
        ...
        print('>', pair[0])
        print('=', pair[1])
        output_words, _ = evaluate(encoder, decoder, pair[0], char2int, phon2int)
